...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "SevenEightSixCrypto is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "SevenEightSixCrypto is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SevenEightSixCrypto is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SevenEightSixCrypto is downloading blocks...");
