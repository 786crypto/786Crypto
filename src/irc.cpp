...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("SevenEightSixCrypto-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SevenEightSixCryptoTEST2\r");
            Send(hSocket, "WHO #SevenEightSixCryptoTEST2\r");
        } else {
            // randomly join #SevenEightSixCrypto00-#SevenEightSixCrypto05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SevenEightSixCrypto%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SevenEightSixCrypto%02d\r", channel_number).c_str());
        }
